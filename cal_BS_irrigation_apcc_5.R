...
for (bsncase in c(7:8)){
     ...
     for (kk in c(1:4)){
          ...
          apcctype="apcc"
          apccpredictdate = read.csv(file.path(predictfilepath, paste0(apcctype,"date.csv")), row.names=1)
          prob.apccrawenssri.dphase.over.ts = read.csv(file.path(predictfilepath, paste0("probforecast_",apcctype,"rawEDP_",dindexname,"_binary_",bsnname,".csv")),
                                                       row.names=1) %>% xts(., ymd(apccpredictdate$x))
          prob.apccupenssri.dphase.over.ts = read.csv(file.path(predictfilepath, paste0("probforecast_",apcctype,"upEDP_",dindexname,"_binary_",bsnname,".csv")),
                                                      row.names=1) %>% xts(., ymd(apccpredictdate$x))
          apccrawensmean.sri.dphase.ts = read.csv(file.path(predictfilepath, paste0("detforecast_",apcctype,"rawEDP_",dindexname,"_",bsnname,".csv")),
                                                  row.names=1) %>% xts(., ymd(apccpredictdate$x))
          apccupensmean.sri.dphase.ts = read.csv(file.path(predictfilepath, paste0("detforecast_",apcctype,"upEDP_",dindexname,"_",bsnname,".csv")),
                                                 row.names=1) %>% xts(., ymd(apccpredictdate$x))
          ...
          rawensmean.sri.dphase.ts = date(rawensmean.sri.dphase.ts) %in% ymd(apccpredictdate$x) %>% rawensmean.sri.dphase.ts[.]
          upensmean.sri.dphase.ts = date(upensmean.sri.dphase.ts) %in% ymd(apccpredictdate$x) %>% upensmean.sri.dphase.ts[.]
          ...
          temp.det.apccraw.dphase = apccrawensmean.sri.dphase.ts[,-1] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          temp.det.apccup.dphase = apccupensmean.sri.dphase.ts[,-1] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          temp.prob.apccraw.dphase = prob.apccrawenssri.dphase.over.ts[,2]
          temp.prob.apccup.dphase = prob.apccupenssri.dphase.over.ts[,2]
          temp.BS = calBS(temp.prob.apccraw.dphase, temp.obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccraw", Predicttype="prob", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp.prob.apccup.dphase, temp.obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccup", Predicttype="prob", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp.det.apccraw.dphase, temp.obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccraw", Predicttype="det", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp.det.apccup.dphase, temp.obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccup", Predicttype="det", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          ...
          temp..det.apccraw.dphase = month(temp.det.apccraw.dphase) %in% period.irr %>% temp.det.apccraw.dphase[.,]
          temp..det.apccup.dphase = month(temp.det.apccup.dphase) %in% period.irr %>% temp.det.apccup.dphase[.,]
          temp..prob.apccraw.dphase = month(temp.prob.apccraw.dphase) %in% period.irr %>% temp.prob.apccraw.dphase[.,]
          temp..prob.apccup.dphase = month(temp.prob.apccup.dphase) %in% period.irr %>% temp.prob.apccup.dphase[.,]
          temp.BS = calBS(temp..prob.apccraw.dphase, temp..obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccraw", Predicttype="prob", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp..prob.apccup.dphase, temp..obs.dphase)
          ...
          temp..det.apccraw.dphase = month(temp.det.apccraw.dphase) %in% period.nonirr %>% temp.det.apccraw.dphase[.,]
          temp..det.apccup.dphase = month(temp.det.apccup.dphase) %in% period.nonirr %>% temp.det.apccup.dphase[.,]
          temp..prob.apccraw.dphase = month(temp.prob.apccraw.dphase) %in% period.nonirr %>% temp.prob.apccraw.dphase[.,]
          temp..prob.apccup.dphase = month(temp.prob.apccup.dphase) %in% period.nonirr %>% temp.prob.apccup.dphase[.,]
          temp.BS = calBS(temp..prob.apccraw.dphase, temp..obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccraw", Predicttype="prob", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp..prob.apccup.dphase, temp..obs.dphase)
          ...
          temp.det.apccraw.dphase = apccrawensmean.sri.dphase.ts[,-1:-2] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          temp.det.apccup.dphase = apccupensmean.sri.dphase.ts[,-1:-2] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          ...
          temp.det.apccraw.dphase = apccrawensmean.sri.dphase.ts[,-1:-3] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          temp.det.apccup.dphase = apccupensmean.sri.dphase.ts[,-1:-3] %>% apply(., 1, sum) %>% xts(., ymd(names(.)))#
          ...
          temp.det.apccraw.dphase = apccrawensmean.sri.dphase.ts[,5]
          temp.det.apccup.dphase = apccupensmean.sri.dphase.ts[,5] 
          temp.prob.apccraw.dphase = prob.apccrawenssri.dphase.over.ts[,5]
          temp.prob.apccup.dphase = prob.apccupenssri.dphase.over.ts[,5]
          temp.BS = calBS(temp.prob.apccraw.dphase, temp.obs.dphase)
          temp.BS.df = data.frame(K=kfold, Period=period, Enstype="apccraw", Predicttype="prob", D=phase, temp.BS[1,])
          BS.df = rbind(BS.df, temp.BS.df)
          temp.BS = calBS(temp.prob.apccup.dphase, temp.obs.dphase)
     ...
     BS.df %>% write.csv(., file.path(BSfilepath, paste0("BS_apcc_allperiod_",dindexname,"_",bsnname,".csv")))
}