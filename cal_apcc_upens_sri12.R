...
win.graph()
year(sri.df$SRI3)%in%year(mean.) %>% sri.df$SRI12[.] %>% plot(.)
lines(mean., col="blue")
...
sqrt(mean((sri.df$SRI12-temp_enssri.stat.df$MEAN)^2))
sqrt(mean((sri.df$SRI12-mean.)^2))
...
for (kk in c(1:4)){
     ...
     for (tt in union(bcalib_idx, bvalid_idx)){
          ...
          as.vector(temp_merged.ts$SRI12[smiidx])*lm_smi2sri12.df$b1[mm] + lm_smi2sri12.df$b0[mm]
          smi.ts[smiidx-1]
          
          upens.est.lm = (as.vector(smi.ts[smiidx-1])-lm_smi2sri12.df$b0[mm])/lm_smi2sri12.df$b1[mm]
          
          upens.sd[tt] = sqrt(1/(1/meansd.$SD[tt]^2 + lm_smi2sri12.df$b1[mm]^2/lm_smi2sri12.df$SD[mm]^2))
          upens.mean[tt] = upens.sd[tt]^2 * (meansd.$MEAN[tt]/meansd.$SD[tt]^2 + upens.est.lm*lm_smi2sri12.df$b1[mm]^2/lm_smi2sri12.df$SD[mm]^2)
     }
     ...
     temp_plotts = merge(temp_merged.ts$SRI12, temp_enssri.stat.df$MEAN, meansd.$MEAN, upens.mean.ts, all=F)
