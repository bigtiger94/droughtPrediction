...
for (bsncase in c(1:8)){
  ...
  temp_lm = lm(SMI ~ SRI12, data=temp_df)
  ...
  lmfilepath = file.path("./likelihood", bsnname, "smi2sri12")
  if(!file.exists(lmfilepath)) dir.create(lmfilepath);
  ...
  #   lm.smi.fitted[tt] = + lm_smi2sri12.df$b0[mm] + temp_smi.ts$SRI12[tt+1] * lm_smi2sri12.df$b1[mm]
  # }
  
  lm.smi.fitted = temp_smi.ts$SRI12 * lm_smi2sri12.df$b1[temp_smi.ts$MONTH] + lm_smi2sri12.df$b0[temp_smi.ts$MONTH]
  ...
  k.b0.df = data.frame(k1=lm_smi2sri12.df$b0)
  k.b1.df = data.frame(k1=lm_smi2sri12.df$b1)
  ...
  k.b0.df = cbind(k.b0.df, lm_smi2sri12.df$b0)
  k.b1.df = cbind(k.b1.df, lm_smi2sri12.df$b1)
...
# 
# 
# loocv.lm.mm = train(SMI~SRI12, method="lm", data=temp_df, trControl=trainControl(method="LOOCV"))
# 
