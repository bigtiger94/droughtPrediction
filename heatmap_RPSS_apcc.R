...
for (bsncase in c(1:length(bsncodes))){
     ...
     RPSS_bsn = read.csv(file.path("./predictResult", paste0("RPSS_apcc_",dindexname,"_",bsnname,".csv")))
...
heatmapname = paste0("RPSS_apcc_all")
heatmapplot = ggplot(RPSS_all.df, aes(x=BSN, y=Case)) + geom_tile(aes(fill=RPSS)) + theme_bw() + ylab("") + ggtitle("")+xlab("")+
     ...
     scale_fill_gradientn(colours=c("red", "white", "blue"), 
                          ...
                          breaks =c(-1, 0, 1),
                          labels=c(-1, 0, 1)) +
     guides(fill = guide_colourbar(barwidth = 1, barheight = 13)) +
...
heatmapname = paste0("RPSS_apcc_irrigation")
...
heatmapname = paste0("RPSS_apcc_non-irrigation")
...
#   'rownames<-'(as.vector(RPSSmean_all$Case)) %>% '<-'(.[c(1,4,2,3),]) %>% 
#   write.csv(., file.path(imgsavepath, paste0("Avg_RPSS_apcc_",dindexname,".csv")))
