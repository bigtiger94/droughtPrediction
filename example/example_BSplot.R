...
BS.df = read.csv(file.path("./predictResult", "BSresult", paste0("BS_allperiod_", dindexname,"_", bsnname,".csv")), row.names=1)
...
temp.BS.df = BS.df %>% dplyr::filter(K==kfold & Period==period) %>%
     dplyr::select(D, Enstype, Predicttype, bs, BSS) %>% mutate(GROUP = paste0(Enstype,Predicttype))
plotname = paste("Brier score for", dindexname, "at", bsnname)
...
temp.bs = temp.BS.df %>% dplyr::filter(Enstype=="up" & Predicttype=="prob") %>% dplyr::select(bs)
lines(c(1:4), temp.bs$bs, col="red", lwd = 2)
temp.bs = temp.BS.df %>% dplyr::filter(Enstype=="up" & Predicttype=="det") %>% dplyr::select(bs)
lines(c(1:4), temp.bs$bs, col="red", lwd = 1, lty=2)

temp.bs = temp.BS.df %>% dplyr::filter(Enstype=="raw" & Predicttype=="prob") %>% dplyr::select(bs)
lines(c(1:4), temp.bs$bs, col="blue", lwd = 2)
temp.bs = temp.BS.df %>% dplyr::filter(Enstype=="raw" & Predicttype=="det") %>% dplyr::select(bs)
lines(c(1:4), temp.bs$bs, col="blue", lwd = 1, lty=2)
axis(1, at=c(1:4), labels = dphase)
grid()

legend("topright", legend=c("Clm.", "EDP (PP)", "EDP+ (PP)", "EDP (DP)", "EDP+ (DP)"),
       lty=c(2,1,1,2,2), col=c("black", "blue", "red", "blue", "red"))
