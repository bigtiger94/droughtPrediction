...
for (kk in c(1:4)){
  ...
  rawensdate = read.csv(file.path(ensfilepath, kfold, "rawensdate.csv"), row.names=1)
  upensdate = read.csv(file.path(ensfilepath, kfold, "upensdate.csv"), row.names=1)
  
  rawens.df = read.csv(file.path(ensfilepath, paste0("meansd_rawens_", dindexname,"_",bsnname,".csv")), row.names=1) %>% xts(., ymd(rawensdate$x))
  upens.df = read.csv(file.path(ensfilepath, kfold, paste0("meansd_upens_", dindexname,"_",bsnname,".csv")), row.names=1) %>% xts(., ymd(upensdate$x))
  ...
  prob.rawenssri.dphase.over.ts %>% write.csv(., file.path(predictsavefilepath, paste0("probforecast_rawEDP_",dindexname,"_binary_",bsnname,".csv")))
  prob.rawenssri.dphase.each.ts %>% write.csv(., file.path(predictsavefilepath, paste0("probforecast_rawEDP_",dindexname,"_multi_",bsnname,".csv")))
  prob.upenssri.dphase.over.ts %>% write.csv(., file.path(predictsavefilepath, paste0("probforecast_upEDP_",dindexname,"_binary_",bsnname,".csv")))
  prob.upenssri.dphase.each.ts %>% write.csv(., file.path(predictsavefilepath, paste0("probforecast_upEDP_",dindexname,"_multi_",bsnname,".csv")))
  rawensmean.sri.dphase.ts %>% write.csv(., file.path(predictsavefilepath, paste0("detforecast_rawEDP_",dindexname,"_",bsnname,".csv")))
  upensmean.sri.dphase.ts %>% write.csv(., file.path(predictsavefilepath, paste0("detforecast_upEDP_",dindexname,"_",bsnname,".csv")))
  obs.sri.dphase.ts %>% write.csv(., file.path(predictsavefilepath, paste0("obs_",dindexname,"_",bsnname,".csv")))
  # date(obs.sri.dphase.ts) %>% write.csv(., file.path(predictsavefilepath, "date.csv"))
}
