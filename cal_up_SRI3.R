#  SRI3 with k1 period (calib: 2000~2013 ; valid: 2014~2017)
dindexname = "sri3"
...
# likelihood function for sri3 (k1) ------------------------------------------------
...
for (tt in union(bcalib_idx, bvalid_idx)){
  ...
  as.vector(temp_merged.ts$SRI3[tt])*lm_smi2sri3.df$b1[mm] + lm_smi2sri3.df$b0[mm]
  temp_merged.ts$SMI[tt-1]
  
  upens.est.lm = (as.vector(temp_merged.ts$SMI[tt-1])-lm_smi2sri3.df$b0[mm])/lm_smi2sri3.df$b1[mm]
  
  upens.sd[tt-12] = sqrt(1/(1/temp_enssri.stat.df$SD[tt]^2 + lm_smi2sri3.df$b1[mm]^2/lm_smi2sri3.df$SD[mm]^2))
  upens.mean[tt-12] = upens.sd[tt-12]^2 * (temp_enssri.stat.df$MEAN[tt]/temp_enssri.stat.df$SD[tt]^2 + upens.est.lm*lm_smi2sri3.df$b1[mm]^2/lm_smi2sri3.df$SD[mm]^2)
}
...
temp_plotts = merge(temp_merged.ts$SRI3, temp_enssri.stat.df$MEAN, upens.mean.ts, all=F)
...
addLegend("topleft", legend.names=c("Obs.", "EDP", "EDP+S"), lty=1, lwd=2,
          col=c("black", "blue", "red"), cex=1.5, bty="o")
# dev.off()
...
# write.csv(upens.stat.df, file.path(enssavefilepath, paste0("meansd_upens_",dindexname,"_",bsnname,".csv")))# write.csv(rownames(upens.stat.df), file.path(enssavefilepath, "upensdate.csv"))
# 
# }